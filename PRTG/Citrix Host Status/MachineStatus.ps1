...
foreach($DeliveryGroup in $DeliveryGroups){
    ...
    $MachinesInMaintenance = $Machines | Where-Object {$_.InMaintenanceMode -eq $True -and $_.PowerState -eq "On" -and $_.DesktopGroupName -eq $DeliveryGroup.Name}
    $MachinesPoweredOff = $Machines | Where-Object {$_.PowerState -eq "Off" -and $_.DesktopGroupName -eq $DeliveryGroup.Name}
    $MachinesUnregistered = $Machines | Where-Object {$_.RegistrationState -eq "Unregistered" -eq $True -and $_.PowerState -eq "On" -and $_.DesktopGroupName -eq $DeliveryGroup.Name}
