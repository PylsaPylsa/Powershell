...
if($False -eq $currentPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)){
    ...
    if((Get-ChildItem "HKLM:\Software\Microsoft\Windows\CurrentVersion\Component Based Servicing\RebootPending" -EA Ignore) -or
    (Get-Item "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\RebootRequired" -EA Ignore) -or
    (Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager" -Name PendingFileRenameOperations -EA Ignore)){
        Write-Host "[!] There is a pending reboot. Please reboot before enabling UWF." -BackgroundColor Red
    }else{
        ...
        if($WindowsEdition -ne "Enterprise"){
		    ...
		    Write-Host "`n[*] Installing UWF Manager. Thanks Daniel Mushailov :)" -ForegroundColor Green
            Copy-Item "$Scriptpath\UWF Manager\" "C:\Program Files\" -Force -Recurse
            & "C:\Program Files\UWF Manager\uwfmon.exe"
            Write-Host "     [!] Done! Default password is `"uwfadmin`"" -ForegroundColor Cyan
