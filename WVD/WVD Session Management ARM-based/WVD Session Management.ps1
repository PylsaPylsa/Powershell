...
{
    $dgvHosts.Rows.Clear()
    $UserSessions = Get-AzWvdUserSession -ResourceGroupName $ResourceGroupName -HostPoolName $HostPool 
    $SessionHosts = Get-AzWvdSessionHost -ResourceGroupName $ResourceGroupName -HostPoolName $HostPool
    $HostpoolProperties = Get-AzWvdHostPool -ResourceGroupName $ResourceGroupName -HostPoolName $HostPool
    ...
    $SessionHosts | foreach{
        ...
        $SessionHostInternalIP = $(Resolve-DnsName -Name $SessionHostName -Type A)[0].IPAddress
        ...
        $rowIndex = $dgvHosts.Rows.Add($SessionHostName,$SessionHostInternalIP,[System.TimeZoneInfo]::ConvertTimeFromUtc($_.LastHeartBeat, $objTimeZone),$SessionHostInternalPingable,$_.Status,$_.AllowNewSession,"$($($UserSessions | Where-Object {$_.Name -like "*$SessionHostName*"}).Count)$MaxSessionLimit")
...
function Fill-Sessions($HostPool)
{
    ...
    $UserSessions | foreach{
        $SessionName = ($_.Name).Split("/")
        $dgvSessions.Rows.Add($_.ActiveDirectoryUserName,$SessionName[2],$SessionName[0],$SessionName[1],[System.TimeZoneInfo]::ConvertTimeFromUtc($_.CreateTime, $objTimeZone),$_.SessionState) | Out-Null
    }
...
function Fill-HostPoolProperties($Hostpool){
    ...
    $dgvHostPoolProperties.Rows.Add("Host Pool Name",$HostpoolProperties.Name) | Out-Null
    ...
    $dgvHostPoolProperties.Rows.Add("Max Session Limit per Host",$HostpoolProperties.MaxSessionLimit) | Out-Null
    $dgvHostPoolProperties.Rows.Add("Load Balancing Mode",$HostpoolProperties.LoadBalancerType) | Out-Null
    $dgvHostPoolProperties.Rows.Add("Development/Validation Environment",$HostpoolProperties.ValidationEnvironment) | Out-Null
    $dgvHostPoolProperties.Rows.Add("Hostpool Type",$HostpoolProperties.HostPoolType) | Out-Null
    $dgvHostPoolProperties.Rows.Add("Custom RDP Properties",$HostpoolProperties.CustomRdpProperty) | Out-Null
}
...
$TenantNode = $tvPools.Nodes.Add($ResourceGroupName)

$Hostpools = Get-AzWvdHostPool -ResourceGroupName $ResourceGroupName | foreach{
...
$tvPools.add_AfterSelect({
    if($this.SelectedNode.Text -ne $ResourceGroupName){
...
$btLogoff.Add_Click({
    ...
    if($Confirm -eq "Yes"){
        ...
        Remove-AzWvdUserSession -ResourceGroupName $ResourceGroupName -HostPoolName $tvPools.SelectedNode.Text -SessionHostName $LogoffSessionHost -Id $LogoffSessionID
...
$btSendMessage.Add_Click({
    ...
    if($Confirm -eq "Yes"){
        ...
        Send-AzWvdUserSessionMessage -ResourceGroupName $ResourceGroupName -HostPoolName $tvPools.SelectedNode.Text -SessionHostName $SendMessageHost -UserSessionId $SendMessageSessionID -MessageTitle $MessageTitle -MessageBody $MessageBody
        Update-AppStatus("Connected to $($tvPools.SelectedNode.Text)")
    }
})
...
$btDrainOn.Add_Click({
   ...
   Update-AzWvdSessionHost -ResourceGroupName $ResourceGroupName -HostPoolName $tvPools.SelectedNode.Text -Name $SessionHostDrainOn -AllowNewSession:$False
...
$lbabout.TabIndex = 0
$lbabout.Text = "By Tom Schoen`nVersion 0.5"
...
$Main.Name = "Main"
$Main.Text = "WVD Session Management [connected to Resource Group $ResourceGroupName]"
